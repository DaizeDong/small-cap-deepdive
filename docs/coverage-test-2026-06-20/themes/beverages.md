...
| — watch band ($2.0–5.0B) | 14 | theme-fit only, no deep-dive (out of small-cap scope) |
...
**Watch band ($2.0–5.0B, theme-fit only, no deep-dive):** two true beverage members were caught here but
...
- **No SIC recall floor (recall risk).** beverages is absent from `THEME_SIC`, so discovery never enumerated
  registrants in beverage SICs (2080–2086) as a floor. Recall depends entirely on the keyword hitting an
