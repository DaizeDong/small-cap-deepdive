...
| ↳ watch band ($2.0 to 5.0B) | 22 | theme-fit only, no deep-dive (band rule) |
...
2. **yfinance vs SEC gross disagreement is the norm, not the exception, for lenders** (12/20). yfinance surfaces balance-sheet aggregates (gross loan book, total debt) that are 3 to 37× the SEC revenue line. The 2.5× cross-source guard correctly treats this as untrustworthy and blocks. This is the single most-fired data guard in the theme.
