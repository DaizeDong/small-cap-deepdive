...
| Market-cap band split | 54 deep / 14 watch | deep = mktcap < $2.0B; watch = $2.0–5.0B |
...
2. **`book != liquidation` discipline.** The NAV path computes tangible equity and bands it at 0.80–1.05×, then applies a 0.6 conviction multiplier (per rubric) — explicitly because book value of a levered MBS/repo portfolio is *not* liquidation value. No mREIT can BUY off book.
