...
- **Unit anomaly in `derived` net-income for JILL ($27,900M shown) and REAL ($41,799M)** — these are clearly scaling glitches in the derived-series display (JILL real NI ≈ $0–30M; REAL is loss-making). Did not affect the valuation block (valuation MoS used the proper market_cap and band) but it does pollute the RANKING net-income column (REAL shows "$41799M NI" while being a known loss-maker). Flagging as a display/units bug in the derived layer.
