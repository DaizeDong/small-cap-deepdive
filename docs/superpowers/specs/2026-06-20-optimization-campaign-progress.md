...
| 阶段 | 状态 | 产出 |
|---|---|---|
| Implement | 🔄 | P-A V形否决(peak_contamination_flag,independent of slope)+ P-B 标签(low_revenue_loss_ratio,不再误报 wrong_entity)+ P-D 崩溃可见 + P-C 路径双重 + P-E 占位剥离 + P-F 分母 + P-G form + P-H ranking 串 |
| Test | ⏳ | 同步重跑 4 个未完成主题 + 回归(NRP 现应 block / INVA·EU 不误杀)|

### 迭代 3 — 扩展能力(规划中,承接 Q2 批准)
P7 二次源 sanity band · P8 SIC 反向召回+recall@gold · P14 取证脊柱 · P11-full 催化剂机制验证 · P15/16/17 firewalled 侧信道。
