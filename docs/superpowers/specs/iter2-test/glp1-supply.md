...
| Band split | deep=93 (≤$2.0B), watch=20 ($2.0–5.0B) | watch-band surfaced, **not** deep-dived per band guard |
...
- **Watchlist (re-underwrite on a drawdown): AMPH.** Amphastar is the one *real, profitable, defensible*
  ...
  another 30–40%, the MoS math flips — this is the name to keep on a pullback list.
