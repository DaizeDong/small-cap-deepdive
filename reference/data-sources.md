...
**What to read:**
- `~/.claude/skills/market-intel/reference/sources-index.md` — master index of sources by category
...
**Availability:** The API key for `twitterapi.io` is stored in `market-intel`'s configuration (`~/.claude/skills/market-intel/secrets/twitterapi-io.env` — not in this repo). The hosted MCP for this key is registered in `~/.claude.json`. Access is via the market-intel catalog read pattern above.
