...
Each dimension: **score 1 to 5** + **evidence Tier** (T1/T2/T3) + **one-line basis**.
...
- `concentration_flag = "watch"` when either ratio falls in the 40 to 60 band (and neither crosses the kill threshold). A `watch` does not block BUY by itself, but must be surfaced in the Dim-3 basis and in §5 kill-flag review.
...
- Historical guidance accuracy: did management hit guidance in the past 4 to 6 quarters?
...
**Note:** This dimension maps to Gate 2 of `discovery-engine.md`. A company that barely passed Gate 2 as `partial` should score 2 to 3 here, not 4 to 5.
...
- lessor_asset_heavy: [True/False] — <one line: leasing/rental SIC OR lease-income concept OR PP&E/lease-fleet ratio + rental revenue — v0.3.2 #8 NOT a kill-flag; a ROUTING flag that forces fcf_cap_model_unsuitable=true → NAV basis (lease-fleet NAV, not trough FCF) even below the 0.62 debt/assets threshold (GBX/RAIL); read mos_basis as "nav"/"abstain" accordingly>
- distress_kill: [True/False] — <one line: distress_score (0-4) + which of neg_ocf / neg_margin / accum_deficit / low_altman fired + distress_altman_z — v0.3.3 CORE-4 PIT distress kill-flag; score>=3 counts in killflag_count and routes the name to AVOID regardless of cheapness; banks/insurers out of scope>
- foreign_filer_unvaluable: [True/False] — <one line: 20-F/40-F filer still empty after the us-gaap+ifrs-full cascade — v0.3.2 #11 explicit "foreign filer — un-valuable from EDGAR" abstain label, not a kill-flag; the null MoS already forces buy_eligible=false via not_assessable_no_intrinsic_band; surface the label so the abstain is clear, never a silent null>
...
MoS basis: <fcf_cap / nav / abstain>   MoS: __%   [NAV MoS: __%]   data_quality flags: <list or none; include low_revenue_loss_ratio, concentration_unquantified, insurance_concepts_present, low_revenue_loss_ratio_extreme, cross_source_mismatch:<detail>, lessor_asset_heavy_fcf_unsuitable_route_nav:<detail> (v0.3.2 #8), foreign_filer_unvaluable:<detail> (v0.3.2 #11) if set>   form_used: <10-K / 20-F / 40-F / null>
...
## T2 DIAGNOSTIC SIGNALS (context only — NOT used in the rating)
<P16 price_divergence: divergence_label + trailing 6m/12m price return>
<P17 ownership: recent 13D/13G + staleness-labeled short interest>
<P15 alt-data, if the agent gathered any: TrendsMCP / GDELT / news-volume, labeled T2>
```

**The T2 section is mandatory and its header is a contract string.** `tools/make_report.py` renders it deterministically from the top-level `signals` namespace and its `--selftest` asserts the header verbatim (`make_report.SIGNALS_HEADER`), so a hand-written report must match it exactly. It sits last, below the rating block and trust banner, behind its own rule, because of the firewall: `valuation.py`, the `buy_eligible` composite, and the BUY trigger read **no** `signals.*` field, and a reader must not mistake this section for a rating driver. Full layer spec: `data-sources.md` ("The Firewalled Diagnostic Side-Channel").
...
### The CORE-4 PIT Distress Kill-Flag, `distress_kill`

This is a **kill-flag**, not a `buy_eligible` term. It enters the rating through the kill-flag count, so it routes a name to AVOID rather than merely blocking a BUY. It is the one predictive de-risk signal in the skill that has been validated out-of-sample.

- **The four flags** (`tools/_deepdive_flags.distress_core4`), all point-in-time fundamentals, no forward or price information, each grounded in distress theory rather than data-mined:
  - `neg_ocf`, operating cash flow < 0
  - `neg_margin`, operating income (EBIT) < 0
  - `accum_deficit`, retained earnings < 0
  - `low_altman`, Altman Z″ (the emerging-market / non-manufacturer variant) `= 6.56·WC/TA + 3.26·RE/TA + 6.72·EBIT/TA + 1.05·Equity/Liab` < 1.1, computed only when every component is present and total assets and total liabilities are both > 0
- **The rank.** `distress_score` is the unweighted count of the four, range 0 to 4. `distress_kill = (distress_score >= 3)`.
- **The consequence.** `distress_kill` counts in `killflag_count` (`make_report._killflag_count`), in the live rating and in the backtest grader alike. Because every BUY path requires zero kill-flags, a distressed name can never rate BUY regardless of how cheap it screens, and `kill-flag count >= 2` routes it to Default AVOID.
- **Scope.** Operating companies only. Banks and insurers are out of scope, their distress is NIM / NPL / deposit-flight, a different model, and they already route to `financial_sic` / abstain upstream.
- **Evidence, stated with the cutoff each number belongs to** (`docs/backtest-2026-06/ROOT_CAUSE_AND_DERISK_EDGE.md`; 25-cell survivorship-safe PIT panel, non-financial, n=412 name-years, 55 forward-12mo blowups below −40%, base rate 13.3%):
  - At the **shipped `score >= 3` kill cutoff**: blowup precision 35.4% vs the 13.3% base, i.e. **lift 2.65x**, at **recall 62%**.
  - At the **per-year top-quintile cutoff** (a different cutoff, reported because the robustness statistics were computed on it): **lift 2.56x** at **recall 51%**, ticker-cluster bootstrap median lift 2.36x, **95% CI [1.73, 3.00]**, P(lift ≤ 1) = 0 over 5,000 ticker resamples.
  - Never pair a lift from one cutoff with a recall from the other. The dose-response is a sharp cliff: score 0 to 2 ≈ 5 to 9% blowup rate, score 3 = 25%, score 4 = 41.7%.
...
**Routing note (v0.3.2 #8, asset-heavy lessors).** A name reaches this branch either because
`total_debt / total_assets > 0.62` OR because `derived.lessor_asset_heavy == true`. The lessor
signal (leasing/rental SIC, a lease-income revenue concept, or a high PP&E/lease-fleet ratio with
rental revenue, see `valuation.md` "Lessor NAV routing") forces `fcf_cap_model_unsuitable = true`
EVEN BELOW the 0.62 threshold, so an asset-heavy lessor that funds its fleet with equity / moderate
leverage (GBX debt/assets = 0.41; RAIL = 0.35) is valued on **lease-fleet NAV, not trough-cycle
FCF**. The data-quality line reads `lessor_asset_heavy_fcf_unsuitable_route_nav:<detail>`. This is a
routing correction only, it moves the name to the NAV path; it never lifts a rating. The NAV BUY
requirements below apply unchanged.
...
- **(b) Cluster open-market insider purchases:** Form 4 filings showing ≥2 to 3 insiders purchasing shares at market prices within any rolling 90-day window. Option exercises, RSU vesting, and any grant-related acquisitions do NOT qualify, only open-market cash purchases.
...
| `lessor_asset_heavy == true` (v0.3.2 #8, leasing/rental SIC `{6726,7377,4741,6159,7359}` OR a lease-income revenue concept OR high PP&E/lease-fleet ratio + rental revenue) | **ROUTING rule, not a kill-flag.** Forces `fcf_cap_model_unsuitable = true` → `mos_basis = "nav"` (or `"abstain"`) EVEN BELOW the 0.62 debt/assets threshold; the asset-heavy lessor (GBX 0.41 / RAIL 0.35) is valued on lease-fleet NAV, not trough-cycle FCF. data_quality: `lessor_asset_heavy_fcf_unsuitable_route_nav:<detail>`. Apply the NAV-path BUY requirements (incl. 0.6 confidence down-weight). Moves the basis; never lifts a rating |
| `foreign_filer_unvaluable == true` (v0.3.2 #11, a 20-F/40-F filer STILL empty after the us-gaap + ifrs-full concept cascade) | **EXPLICIT abstain label, not a separate gate.** Surfaced in `data_quality` as `foreign_filer_unvaluable:<detail>` so the abstain reads "foreign filer, un-valuable from EDGAR" instead of a silent `intrinsic_band_null`. The null MoS already forces `buy_eligible = false` via `not_assessable_no_intrinsic_band` (#9). Graceful abstain, never a crash, never a false BUY |
...
| `distress_kill == true` (v0.3.3, CORE-4 PIT distress `distress_score >= 3`: count of `neg_ocf`, `neg_margin` (operating loss), `accum_deficit` (retained earnings < 0), `low_altman` (Altman Z″ < 1.1)) | **KILL-FLAG**, counted in `killflag_count` in both the live rating and the backtest grader → **routes the name to AVOID regardless of cheapness**. The one OOS-validated predictive de-risk signal: at the shipped `score >= 3` cutoff, blowup precision 35.4% vs 13.3% base (**lift 2.65x**) at recall 62%; the separate per-year top-quintile cutoff measures lift 2.56x at recall 51% with a ticker-cluster bootstrap 95% CI of [1.73, 3.00]. Banks/insurers out of scope (route to `financial_sic`/abstain upstream). Spec: "The CORE-4 PIT Distress Kill-Flag" above; evidence: `docs/backtest-2026-06/ROOT_CAUSE_AND_DERISK_EDGE.md` |
...
| `going_concern` fires in the most recent 10-K or 10-Q | **Eliminated before deep-dive at the `cheap_pass` gate**, so the name never reaches this rubric. `cheap_pass.py` sets `rejected = True` on `reject_going_concern` (as it does on `killflag_count >= 2`, `reject_burn`, and `concentration_flag == "kill"`), and `run_theme.py` forwards only `~rejected` rows. Do not deep-dive an eliminated candidate |
...
| Single customer / program in the 40 to 60% band (`concentration_flag == "watch"`) | Surface in Dim 3 basis + §5 kill-flag review; does not block BUY by itself |
