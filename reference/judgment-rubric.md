...
MoS basis: <fcf_cap / nav / abstain>   MoS: __%   [NAV MoS: __%]   data_quality flags: <list or none; include low_revenue_loss_ratio, concentration_unquantified, insurance_concepts_present, low_revenue_loss_ratio_extreme if set>   form_used: <10-K / 20-F / 40-F / null>
...
`buy_eligible = (not extreme_mos_review_required) AND (not large_cap_out_of_scope) AND (not fcf_sustainability_uncertain) AND (not financial_sic forced-unsuitable) AND (not debt_truncation_suspected) AND (not wrong_entity_suspected) AND (concentration_flag != "kill") AND (not fundamental_decline_flag) AND (not peak_contamination_flag) AND (not insurance_concepts_present) AND (not low_revenue_loss_ratio_extreme)`

Iteration 3 adds two terms to the composite: `(not insurance_concepts_present)` (A3) and `(not low_revenue_loss_ratio_extreme)` (A4). Both are detailed below.
