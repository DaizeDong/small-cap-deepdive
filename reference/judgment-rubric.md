...
`buy_eligible = (not extreme_mos_review_required) AND (not large_cap_out_of_scope) AND (not fcf_sustainability_uncertain) AND (not financial_sic forced-unsuitable) AND (not debt_truncation_suspected) AND (not wrong_entity_suspected) AND (concentration_flag != "kill") AND (not fundamental_decline_flag) AND (not peak_contamination_flag) AND (not insurance_concepts_present) AND (not low_revenue_loss_ratio_extreme) AND (not cross_source_mismatch)`

Iteration 3 adds two terms to the composite: `(not insurance_concepts_present)` (A3) and `(not low_revenue_loss_ratio_extreme)` (A4). Iteration 5 adds one more: `(not cross_source_mismatch)` (P7 — the second-source sanity band, the first composite term sourced from an INDEPENDENT feed rather than internal-consistency on SEC XBRL). All are detailed below.
