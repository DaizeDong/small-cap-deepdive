...
| `confidence` | `number\|null` (0–1) | Model confidence at verdict time. Mapped to `implied_prob` by rating direction (see Rating → Implied Probability). `null` → fixed `RATING_PROB` convention is used. |
| `implied_prob` | `number` (0–1) | Model probability the thesis resolves favorably (stock beats benchmark over horizon). Derived from `confidence` mapped by rating direction; falls back to `RATING_PROB` when `confidence` is null. |
